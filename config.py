...
class Config:
    ...
    RESOURCE_FILE_PATH = Path(
        os.getenv("RESOURCE_FILE_PATH") or Path(__file__).parent.joinpath("resources")
    )
    ...
    PUBSUB_INVALID_CASE_TOPIC = os.getenv(
        "PUBSUB_INVALID_CASE_TOPIC", "event_invalid-case"
    )
    PUBSUB_PRINT_FULFILMENT_TOPIC = os.getenv(
        "PUBSUB_PRINT_FULFILMENT_TOPIC", "event_print-fulfilment"
    )
    PUBSUB_EQ_LAUNCH_TOPIC = os.getenv("PUBSUB_EQ_LAUNCH_TOPIC", "event_eq-launch")
    PUBSUB_DEACTIVATE_UAC_TOPIC = os.getenv(
        "PUBSUB_DEACTIVATE_UAC_TOPIC", "event_deactivate-uac"
    )
    PUBSUB_UPDATE_SAMPLE_TOPIC = os.getenv(
        "PUBSUB_UPDATE_SAMPLE_TOPIC", "event_update-sample"
    )
    PUBSUB_UPDATE_SAMPLE_SENSITIVE_TOPIC = os.getenv(
        "PUBSUB_UPDATE_SAMPLE_SENSITIVE_TOPIC", "event_update-sample-sensitive"
    )
    PUBSUB_OUTBOUND_UAC_SUBSCRIPTION = os.getenv(
        "PUBSUB_OUTBOUND_UAC_SUBSCRIPTION", "event_uac-update_rh_at"
    )
    PUBSUB_OUTBOUND_CASE_SUBSCRIPTION = os.getenv(
        "PUBSUB_OUTBOUND_CASE_SUBSCRIPTION", "event_case-update_rh_at"
    )
    PUBSUB_OUTBOUND_SURVEY_SUBSCRIPTION = os.getenv(
        "PUBSUB_OUTBOUND_SURVEY_SUBSCRIPTION", "event_survey-update_rh_at"
    )
    PUBSUB_OUTBOUND_COLLECTION_EXERCISE_SUBSCRIPTION = os.getenv(
        "PUBSUB_OUTBOUND_COLLECTION_EXERCISE_SUBSCRIPTION",
        "event_collection-exercise-update_rh_at",
    )
    PUBSUB_NEW_CASE_TOPIC = os.getenv("PUBSUB_NEW_CASE_TOPIC", "event_new-case")
    PUBSUB_CLOUD_TASK_QUEUE_AT_SUBSCRIPTION = os.getenv(
        "PUBSUB_CLOUD_TASK_QUEUE_AT_SUBSCRIPTION", "cloud_task_queue_at"
    )
    PUBSUB_NIFI_INTERNAL_PRINT_NOTIFICATION_SUBSCRIPTION = os.getenv(
        "PUBSUB_NIFI_INTERNAL_PRINT_NOTIFICATION_SUBSCRIPTION",
        "export_file_nifi_notification",
    )
    ...
    EXCEPTIONMANAGER_CONNECTION_HOST = os.getenv(
        "EXCEPTIONMANAGER_CONNECTION_HOST", "localhost"
    )
    EXCEPTIONMANAGER_CONNECTION_PORT = os.getenv(
        "EXCEPTIONMANAGER_CONNECTION_PORT", "8666"
    )
    EXCEPTION_MANAGER_URL = (
        f"http://{EXCEPTIONMANAGER_CONNECTION_HOST}:{EXCEPTIONMANAGER_CONNECTION_PORT}"
    )
    ...
    if SUPPORT_TOOL_PORT == "80":
        ...
        SUPPORT_TOOL_API = (
            f"{SUPPORT_TOOL_PROTOCOL}://{SUPPORT_TOOL_HOST}:{SUPPORT_TOOL_PORT}/api"
        )
        SUPPORT_TOOL_URL = (
            f"{SUPPORT_TOOL_PROTOCOL}://{SUPPORT_TOOL_HOST}:{SUPPORT_TOOL_PORT}"
        )
    ...
    EXPORT_FILE_DESTINATION_CONFIG_JSON_PATH = Path(
        os.getenv("EXPORT_FILE_DESTINATION_CONFIG_JSON_PATH")
        or Path(__file__).parent.joinpath("dummy_export_file_destination_config.json")
    )
    EXPORT_FILE_DESTINATIONS_CONFIG = (
        json.loads(EXPORT_FILE_DESTINATION_CONFIG_JSON_PATH.read_text())
        if EXPORT_FILE_DESTINATION_CONFIG_JSON_PATH
        and EXPORT_FILE_DESTINATION_CONFIG_JSON_PATH.exists()
        else None
    )
    FILE_UPLOAD_DESTINATION = os.getenv(
        "FILE_UPLOAD_DESTINATION", str(Path.home().joinpath("Documents/export_files"))
    )
    FILE_UPLOAD_MODE = os.getenv("FILE_UPLOAD_MODE", "LOCAL")
    OUR_EXPORT_FILE_DECRYPTION_KEY = os.getenv(
        "OUR_EXPORT_FILE_DECRYPTION_KEY",
        str(RESOURCE_FILE_PATH.joinpath("dummy_keys", "dummy-key-ssdc-rm-private.asc")),
    )
    OUR_EXPORT_FILE_DECRYPTION_KEY_PASSPHRASE = os.getenv(
        "OUR_EXPORT_FILE_DECRYPTION_KEY_PASSPHRASE", "test"
    )
    ...
    EQ_DECRYPTION_JSON_FILE = Path(
        os.getenv("EQ_JSON_FILE")
        or RESOURCE_FILE_PATH.joinpath("dummy_keys", "eq-decrypt-keys.json")
    )
    ...
    CODE_GUIDE_MARKDOWN_FILE_PATH = Path(
        os.getenv("CODE_GUIDE_MARKDOWN_FILE_PATH")
    ...
    SUPPLIER_INTERNAL_REPROGRAPHICS = os.getenv(
        "SUPPLIER_INTERNAL_REPROGRAPHICS", "internal_reprographics"
    )
