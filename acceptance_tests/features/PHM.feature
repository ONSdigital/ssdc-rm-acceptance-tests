Feature: SRM supports PHM shape surveys
  ...
  Scenario: A (prototype) PHM "wave" pattern can be scheduled with action rules
    ...
    And an export file template has been created with template ["FULL_NAME","ADDRESS_LINE1","ADDRESS_LINE2","POSTCODE","__uac__"]
    Then an export file action rule has been created with UAC metadata "{"wave":"1","test_survey":true}" and classifiers "sample_sensitive ->> 'EMAIL' = '' AND (SELECT COUNT(*) FROM casev3.uac_qid_link WHERE caze_id = casev3.cases.id AND metadata ->> 'wave' = '1' AND (metadata ->> 'opt_out')::boolean AND NOT receipt_received) > 0"
    And 2 UAC_UPDATE messages are emitted with active set to true
    And an export file is created with correct rows
    And an export file template has been created with template ["FULL_NAME","ADDRESS_LINE1","ADDRESS_LINE2","POSTCODE"]
    Then an export file action rule has been created with classifiers "sample_sensitive ->> 'EMAIL' = '' AND (SELECT COUNT(*) FROM casev3.uac_qid_link WHERE caze_id = casev3.cases.id AND metadata ->> 'wave' = '1' AND (metadata ->> 'test_survey')::boolean AND NOT receipt_received) > 0"
    And an export file is created with correct rows