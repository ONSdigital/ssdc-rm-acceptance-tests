Feature: SRM supports PHM shape surveys
  ...
  Scenario: A (prototype) PHM "wave" pattern can be scheduled with action rules
    ...
    And 3 UAC_UPDATE messages are emitted with active set to true
    ...
    Given an export file template has been created with template ["FULL_NAME","ADDRESS_LINE1","ADDRESS_LINE2","POSTCODE","__uac__"]
    ...
    Given an export file template has been created with template ["FULL_NAME","ADDRESS_LINE1","ADDRESS_LINE2","POSTCODE","__uac__"]
    Then an export file action rule has been created with UAC metadata "{"wave":"1","test_survey":true,"reminder":true}" and classifiers "sample_sensitive ->> 'EMAIL' = '' AND EXISTS (SELECT 1 FROM casev3.uac_qid_link WHERE caze_id = casev3.cases.id AND metadata ->> 'wave' = '1' AND (metadata ->> 'test_survey')::boolean AND NOT receipt_received) AND NOT EXISTS (SELECT 1 FROM casev3.uac_qid_link WHERE caze_id = casev3.cases.id AND metadata ->> 'wave' = '1' AND (metadata ->> 'test_survey')::boolean AND receipt_received)"
    And 3 UAC_UPDATE messages are emitted with active set to true
    And an export file is created with correct rows
