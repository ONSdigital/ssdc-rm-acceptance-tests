Feature: Check exception manager is called for every topic and handles them as expected
  ...
  Scenario: Deactivate unknown UAC turns up in exception manager
    When a bad deactivate uac message is put on the topic with email address "BadDeactivate"
  ...
  Scenario: Bad receipt message turns up in exception manager
    When a bad receipt message is put on the topic with email address "BadReceipt"
  ...
  Scenario: Bad refusal message turns up in exception manager
    When a bad REFUSAL event is put on the topic with email address "BadRefusal"
  ...
  Scenario: Bad respondent authenticated message turns up in exception manager
    When a bad respondent authenticated event is put on the topic with email address "BadRespondentAuthenticated"
  ...
  Scenario: Bad sensitive data message turns up in exception manager
    When a bad sensitive data event is put on the topic with email address "BadSensitiveData"
  ...
  Scenario: Bad survey launched message turns up in exception manager
    When a bad survey launched event is put on the topic with email address "BadSurveyLaunched"
